...
public class SchedulingPattern
{
	...
	protected ArrayList<ValueMatcherCollection> valueMatchers = new ArrayList<ValueMatcherCollection>();
	...
	public boolean match(TimeZone timezone, long millis)
	{
		...
		for(int i = 0; i < valueMatchers.size(); i++)
		{
			...
			boolean eval = matcher.minute.match(minute)
					&& matcher.hour.match(hour) && ((matcher.dayOfMonth instanceof DayOfMonthValueMatcher)
							? ((DayOfMonthValueMatcher) matcher.dayOfMonth).match(dayOfMonth, month, gc.isLeapYear(year)) : matcher.dayOfMonth.match(dayOfMonth))
